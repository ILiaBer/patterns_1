...
public class TestPatterns {
    ...
    void shouldSendCorrectForm() {
        ...
        $("[class='notification__content']").shouldHave(text("Встреча успешно забронирована на " + Data.generateDate(3)), Duration.ofSeconds(20));
        $("[data-test-id=date] input").doubleClick().sendKeys(Data.generateDate(6));
        $$("button").find(exactText("Забронировать")).click();
        $("[class='notification__content']").shouldHave(text("Встреча успешно забронирована на " + Data.generateDate(6)), Duration.ofSeconds(20));
